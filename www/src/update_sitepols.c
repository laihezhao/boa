...
static SITEPOLS sql_sitepols[1];
...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_sitepols[0].id,		argv[0] ? argv[0] : "");
		strcpy(sql_sitepols[0].polid,		argv[1] ? argv[1] : "");
		strcpy(sql_sitepols[0].polname,		argv[2] ? argv[2] : "");
		strcpy(sql_sitepols[0].devname,		argv[3] ? argv[3] : "");
		strcpy(sql_sitepols[0].devaddr,		argv[4] ? argv[4] : "");
		strcpy(sql_sitepols[0].funcode,		argv[5] ? argv[5] : "");
		strcpy(sql_sitepols[0].datatype,	argv[6] ? argv[6] : "");
		strcpy(sql_sitepols[0].regaddr,		argv[7] ? argv[7] : "");
		strcpy(sql_sitepols[0].reglen,		argv[8] ? argv[8] : "");
		strcpy(sql_sitepols[0].databit,		argv[9] ? argv[9] : "");
	//	strcpy(sql_sitepols[0].polvalue,	argv[10] ? argv[10] : "");
		strcpy(sql_sitepols[0].upvalue,		argv[10] ? argv[10] : "");
		strcpy(sql_sitepols[0].lowvalue,	argv[11] ? argv[11] : "");
		strcpy(sql_sitepols[0].k,	        argv[12] ? argv[12] : "");
		strcpy(sql_sitepols[0].b,	        argv[13] ? argv[13] : "");
		strcpy(sql_sitepols[0].alarmtype,	argv[14] ? argv[14] : "");
	//	strcpy(sql_sitepols[0].flag,	        argv[16] ? argv[16] : "");
...
static int select_table(int n)
{
	...
	snprintf(sql,sizeof(sql),"SELECT id,polid,polname,devname,devaddr,funcode,datatype,regaddr,reglen,\
...
static void show_update(void)
{
	...
	printf("<td><input type=\"text\" name=\"update_id\" style=\"width:110px;\" value=\"%s\" class=\"inputxt\" \
        ...
        printf("</td>\n");
	printf("<td width=\"13%%\" align=\"right\" nowrap=\"nowrap\" bgcolor=\"#f1f1f1\">仪表ID：</td>\n");
	printf("<td><input type=\"text\" name=\"update_polid\" style=\"width:110px;\" value=\"%s\" class=\"inputxt\" \
			datatype=\"*1-32\" nullmsg=\"请输入仪表ID！\" errormsg=\"仪表ID1-32任意字符!\" /></td>\n",sql_sitepols[0].polid);
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">仪表ID1-32任意字符!</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
	printf("</tr>\n");
	printf("</table>\n");
	...
	printf("<td><input type=\"text\" name=\"update_polname\" style=\"width:110px;\" value=\"%s\" class=\"inputxt\" \
			datatype=\"*1-32\" nullmsg=\"请输入仪表名称！\" errormsg=\"仪表名称1-32位任意字符！\" /></td>\n",sql_sitepols[0].polname);
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">仪表名称1-32位任意字符！</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
