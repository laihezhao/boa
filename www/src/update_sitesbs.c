...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_sitesbs[0].id,	argv[0] ? argv[0] : "");	
		strcpy(sql_sitesbs[0].sbid,	argv[1] ? argv[1] : "");
		strcpy(sql_sitesbs[0].sbname,	argv[2] ? argv[2] : "");
		strcpy(sql_sitesbs[0].sbtype,	argv[3] ? argv[3] : "");
		strcpy(sql_sitesbs[0].description,	argv[4] ? argv[4] : "");
	}
...
static void show_update(void)
{
	...
	printf("<td><input type=\"text\" name=\"update_sbname\" style=\"width:110px;\" value=\"%s\" class=\"inputxt\" \
			datatype=\"*1-20\" nullmsg=\"请输入设备名称！\" errormsg=\"设备名称2-10位0任意字符！\" /></td>\n",sql_sitesbs[0].sbname);
        printf("<td>\n");
	...
	printf("<td><input type=\"text\" name=\"update_sbtype\" style=\"width:110px;\" value=\"%s\" class=\"inputxt\" \
			datatype=\"n1-1\" nullmsg=\"请输入设备类型！\" errormsg=\"设备类型(1：泵类，2：阀门，3：SB00)!\" /></td>\n",sql_sitesbs[0].sbtype);
        printf("<td>\n");
	...
	printf("<td><input type=\"text\" name=\"update_description\" style=\"width:200px;\" value=\"%s\" class=\"inputxt\" \
			datatype=\"*1-64\" nullmsg=\"请输入设备描述！\" errormsg=\"设备描述！\" /></td>\n",sql_sitesbs[0].description);
        printf("<td>\n");
