...
static char sql_HOSTNAME[MAX_BUF_SIZE];
static char sql_IP[MAX_BUF_SIZE];
static char sql_PORT[MAX_BUF_SIZE];
static char sql_ENABLE[MAX_BUF_SIZE];
...
static void init_memory(void)
{
	...
	memset(sql_HOSTNAME, 0,     sizeof(sql_HOSTNAME));
	memset(sql_IP,       0,     sizeof(sql_IP));
	memset(sql_PORT,     0,     sizeof(sql_PORT));
	memset(sql_ENABLE,   0,     sizeof(sql_ENABLE));
}
/*obtain the update value*/
static void update_value(void)
{
	get_form_value("update_id",      sql_ID,      sizeof(sql_ID));
	get_form_value("update_hostname",sql_HOSTNAME,sizeof(sql_HOSTNAME));
	get_form_value("update_ip",      sql_IP,      sizeof(sql_IP));
	get_form_value("update_port",    sql_PORT,    sizeof(sql_PORT));
	get_form_value("update_enable",  sql_ENABLE,  sizeof(sql_ENABLE));
}
...
static void insert_value(void)
{
	...
	get_form_value("HOSTNAME",    sql_HOSTNAME,sizeof(sql_HOSTNAME));
	get_form_value("IP",          sql_IP,      sizeof(sql_IP));
	get_form_value("PORT",        sql_PORT,    sizeof(sql_PORT));
	get_form_value("ENABLE",      sql_ENABLE,  sizeof(sql_ENABLE));
}
...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_server[row].id,	argv[0] ? argv[0] : "");
		strcpy(sql_server[row].hostname,argv[1] ? argv[1] : "");
		strcpy(sql_server[row].ip,	argv[2] ? argv[2] : "");
		strcpy(sql_server[row].port,	argv[3] ? argv[3] : "");
		strcpy(sql_server[row].enable,	argv[4] ? argv[4] : "");
	}
...
static int update_table(void)
{
	...
	snprintf(sql,sizeof(sql),"UPDATE %s SET hostname = \'%s\',ip = \'%s\',port = %d, enable=%d where ID = %d;",\
			TABLENAME,sql_HOSTNAME,sql_IP,atoi(sql_PORT),atoi(sql_ENABLE), atoi(sql_ID));
...
static int insert_table(void)
{
	...
	snprintf(sql,sizeof(sql),"INSERT INTO %s VALUES (%d,\'%s\',\'%s\',%d,%d);",TABLENAME,atoi(sql_ID),\
			sql_HOSTNAME,sql_IP,atoi(sql_PORT), atoi(sql_ENABLE));
...
static void show_form(void)
{
	...
	printf("window.location = \"/cgi-bin/update_server.cgi?id=\" + id;\n");
        printf("}\n");
    	printf("</script>\n");
	...
	printf("});\n");
        ...
        printf("window.location = \"/cgi-bin/server.cgi?Id=\" + Id;\n");
        printf("}\n");
        printf("}\n");
	...
	printf("<div class=\"title_right\"><span class=\"pull-right margin-bottom-5\"> \
			...
			<i class=\"icon-plus icon-white\"></i> 添加中心信息</a> \
			</span><strong>中心信息</strong></div>\n");
	...
	printf("<td><input type=\"text\" name=\"HOSTNAME\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
			datatype=\"*1-64\" nullmsg=\"请输入中心名称！\" errormsg=\"中心名称1-20任意字符!\" /></td>\n");
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">中心名称1-20任意字符!</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
	printf("</tr>\n");
	printf("</table>\n");
	...
	printf("<td><input type=\"text\" name=\"IP\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
			datatype=\"*1-32\" nullmsg=\"请输入IP地址！\" errormsg=\"IP地址1-32位0任意字符！\" /></td>\n");
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">IP地址1-32位0任意字符！</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
	...
	printf("<td><input type=\"text\" name=\"ENABLE\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
			datatype=\"n1-1\" nullmsg=\"请输入使能位！\" errormsg=\"使能位(0/1)！\" /></td>\n");
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">使能位(0/1)！</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
	...
	printf("<td nowrap=\"nowrap\"><strong>中心名称</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>IP地址</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>端口号</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>使能位</strong></td>\n");
