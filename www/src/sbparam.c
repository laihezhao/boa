...
static char sql_PARAMNAME[MAX_BUF_SIZE];
...
static SBPARAM sql_sbparam[100];
...
static void init_memory(void)
{
	...
	memset(sql_PARAMNAME,0, sizeof(sql_PARAMNAME));
...
static void update_value(void)
{
	...
	get_form_value("update_paramname",sql_PARAMNAME,sizeof(sql_PARAMNAME));
...
static void insert_value(void)
{
	...
	get_form_value("PARAMNAME",sql_PARAMNAME,sizeof(sql_PARAMNAME));
...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_sbparam[row].id,		argv[0] ? argv[0] : "");
		strcpy(sql_sbparam[row].sbid,		argv[1] ? argv[1] : "");
		strcpy(sql_sbparam[row].paramname,	argv[2] ? argv[2] : "");
		strcpy(sql_sbparam[row].devname,	argv[3] ? argv[3] : "");
		strcpy(sql_sbparam[row].devaddr,	argv[4] ? argv[4] : "");
		strcpy(sql_sbparam[row].funcode,	argv[5] ? argv[5] : "");
		strcpy(sql_sbparam[row].datatype,	argv[6] ? argv[6] : "");
		strcpy(sql_sbparam[row].regaddr,	argv[7] ? argv[7] : "");
		strcpy(sql_sbparam[row].reglen,		argv[8] ? argv[8] : "");
		strcpy(sql_sbparam[row].databit,	argv[9] ? argv[9] : "");
		strcpy(sql_sbparam[row].polvalue,	argv[10] ? argv[10] : "");
		strcpy(sql_sbparam[row].upvalue,	argv[11] ? argv[11] : "");
		strcpy(sql_sbparam[row].lowvalue,	argv[12] ? argv[12] : "");
		strcpy(sql_sbparam[row].k,	        argv[13] ? argv[13] : "");
		strcpy(sql_sbparam[row].b,	        argv[14] ? argv[14] : "");
		strcpy(sql_sbparam[row].alarmtype,	argv[15] ? argv[15] : "");
	}
...
static int update_table(void)
{
	...
	snprintf(sql,sizeof(sql),"UPDATE %s SET sbid=\'%s\',paramname=\'%s\',devname=\'%s\',devaddr=%d,\
			funcode=\'%s\',datatype=%d,regaddr=%d, reglen=%d,databit=%d,upvalue=\'%s\',\
			lowvalue=\'%s\',K=\'%s\',B=\'%s\',alarmtype=%d where ID=%d;",TABLENAME,sql_SBID,sql_PARAMNAME,\
...
static int insert_table(void)
{
	...
	snprintf(sql,sizeof(sql),"INSERT INTO %s (id,sbid,paramname,devname,devaddr,funcode,datatype,regaddr,reglen,\
							databit,upvalue,lowvalue,K,B,alarmtype) \
						VALUES (%d,\'%s\',\'%s\',\'%s\',%d,\'%s\',%d,%d,%d,%d,\'%s\',\'%s\',\'%s\',\'%s\',%d);",\
					TABLENAME,atoi(sql_ID),sql_SBID,sql_PARAMNAME,sql_DEVNAME,atoi(sql_DEVADDR), sql_FUNCODE,\
...
static void show_form(void)
{
	...
	printf("window.location = \"/cgi-bin/update_sbparam.cgi?id=\" + id;\n");
        printf("}\n");
    	printf("</script>\n");
	...
	printf("});\n");
        ...
        printf("window.location = \"/cgi-bin/sbparam.cgi?Id=\" + Id;\n");
        printf("}\n");
        printf("}\n");
	printf("</script>\n");
	...
	printf("<div class=\"title_right\"><span class=\"pull-right margin-bottom-5\"> \
			...
			<i class=\"icon-plus icon-white\"></i> 添加设备参数</a> \
			</span><strong>设备参数</strong></div>\n");
	...
	printf("<td><input type=\"text\" name=\"ID\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
        ...
        printf("</td>\n");
	printf("<td width=\"15%%\" align=\"right\" nowrap=\"nowrap\" bgcolor=\"#f1f1f1\">设备ID：</td>\n");
	printf("<td><input type=\"text\" name=\"SBID\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
			datatype=\"*1-32\" nullmsg=\"请输入设备ID！\" errormsg=\"设备ID!\" /></td>\n");
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">设备ID!</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
	printf("</tr>\n");
	printf("</table>\n");
	...
	printf("<table class=\"table table-bordered\" style=\"margin-bottom:0px;\">\n");
	printf("<tr>\n");
	printf("<td width=\"15%%\" align=\"right\" nowrap=\"nowrap\" bgcolor=\"#f1f1f1\">参数名称：</td>\n");
	printf("<td><input type=\"text\" name=\"PARAMNAME\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
			datatype=\"*1-32\" nullmsg=\"请输入参数名称！\" errormsg=\"仪表名称！\" /></td>\n");
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">参数名称！</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
	...
	printf("<td nowrap=\"nowrap\"><strong>参数名称</strong></td>\n");
	...
	for (i = 0;i < row;i++)
	{
		...
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].sbid);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].paramname);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].devname);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].devaddr);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].funcode);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].datatype);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].regaddr);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].reglen);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].databit);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].upvalue);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].lowvalue);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].k);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].b);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sbparam[i].alarmtype);
	        printf("<td nowrap=\"nowrap\"><a href=\"javascript:void(0)\" onclick=\"Dele(%s)\">删除</a> \
						<a href=\"javascript:void(0)\" onclick=\"Edit(%s)\">修改</a></td>\n",\
						sql_sbparam[i].id,sql_sbparam[i].id);
		printf("</tr>\n");	
	}
