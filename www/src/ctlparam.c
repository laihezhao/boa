...
static CTLPARAM sql_ctlparam[100];
...
static void init_memory(void)
{
	memset(sql_ctlparam, 0, sizeof(sql_ctlparam));
...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_ctlparam[row].id,		argv[0] ? argv[0] : "");
		
		strcpy(sql_ctlparam[row].sbid,		argv[1] ? argv[1] : "");
		
		strcpy(sql_ctlparam[row].paramname,	argv[2] ? argv[2] : "");
	
		strcpy(sql_ctlparam[row].devname,	argv[3] ? argv[3] : "");
	
		strcpy(sql_ctlparam[row].devaddr,	argv[4] ? argv[4] : "");
		
		strcpy(sql_ctlparam[row].funcode,	argv[5] ? argv[5] : "");
		
		strcpy(sql_ctlparam[row].datatype,	argv[6] ? argv[6] : "");
		
		strcpy(sql_ctlparam[row].regaddr,	argv[7] ? argv[7] : "");
		
		strcpy(sql_ctlparam[row].reglen,	argv[8] ? argv[8] : "");

		strcpy(sql_ctlparam[row].databit,	argv[9] ? argv[9] : "");
...
static int update_table(void)
{
	...
	snprintf(sql,sizeof(sql),"UPDATE %s SET sbid=\'%s\',paramname=\'%s\',devname=\'%s\',devaddr=%d,\
			funcode=\'%s\',datatype=%d,regaddr=%d, reglen=%d,databit=%d where ID=%d;",TABLENAME,sql_SBID,sql_PARAMNAME,\
			sql_DEVNAME,atoi(sql_DEVADDR),sql_FUNCODE,atoi(sql_DATATYPE),atoi(sql_REGADDR),atoi(sql_REGLEN),\
			atoi(sql_DATABIT),atoi(sql_ID));
...
static int insert_table(void)
{
	...
	snprintf(sql,sizeof(sql),"INSERT INTO %s (id,sbid,paramname,devname,devaddr,funcode,datatype,regaddr,reglen,databit) \
						VALUES (%d,\'%s\',\'%s\',\'%s\',%d,\'%s\',%d,%d,%d,%d);",\
					TABLENAME,atoi(sql_ID),sql_SBID,sql_PARAMNAME,sql_DEVNAME,atoi(sql_DEVADDR), sql_FUNCODE,\
					atoi(sql_DATATYPE),atoi(sql_REGADDR),atoi(sql_REGLEN), atoi(sql_DATABIT));
...
static void show_form(void)
{
	...
	printf("window.location = \"/cgi-bin/update_ctlparam.cgi?id=\" + id;\n");
        printf("}\n");
    	printf("</script>\n");
	...
	printf("});\n");
        ...
        printf("window.location = \"/cgi-bin/ctlparam.cgi?Id=\" + Id;\n");
        printf("}\n");
        printf("}\n");
	printf("</script>\n");
	...
	printf("<div class=\"title_right\"><span class=\"pull-right margin-bottom-5\"> \
			...
			<i class=\"icon-plus icon-white\"></i> 添加控制参数</a> \
			</span><strong>控制参数</strong></div>\n");
	...
	printf("<form class=\"registerform\" action=\"/cgi-bin/ctlparam.cgi\">\n");
	...
	for (i = 0;i < row;i++)
	{
		...
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_ctlparam[i].sbid);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_ctlparam[i].paramname);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_ctlparam[i].devname);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_ctlparam[i].devaddr);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_ctlparam[i].funcode);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_ctlparam[i].datatype);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_ctlparam[i].regaddr);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_ctlparam[i].reglen);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_ctlparam[i].databit);
	        printf("<td nowrap=\"nowrap\"><a href=\"javascript:void(0)\" onclick=\"Dele(%s)\">删除</a> \
						<a href=\"javascript:void(0)\" onclick=\"Edit(%s)\">修改</a></td>\n",\
						sql_ctlparam[i].id,sql_ctlparam[i].id);
		printf("</tr>\n");
