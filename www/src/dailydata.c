...
static DAILYDATA sql_dailydata[100];
...
static void init_memory(void)
{
	memset(sql_dailydata,0, sizeof(sql_dailydata));
}
...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_dailydata[row].id,		argv[0] ? argv[0] : "");
		strcpy(sql_dailydata[row].datatime,	argv[1] ? argv[1] : "");
		strcpy(sql_dailydata[row].polid,	argv[2] ? argv[2] : "");
		strcpy(sql_dailydata[row].pol_min,	argv[3] ? argv[3] : "");
		strcpy(sql_dailydata[row].pol_avg,	argv[4] ? argv[4] : "");
		strcpy(sql_dailydata[row].pol_max,	argv[5] ? argv[5] : "");
		strcpy(sql_dailydata[row].pol_col,	argv[6] ? argv[6] : "");
		strcpy(sql_dailydata[row].centre1,	argv[7] ? argv[7] : "");
		strcpy(sql_dailydata[row].centre2,	argv[8] ? argv[8] : "");
		strcpy(sql_dailydata[row].centre3,	argv[9] ? argv[9] : "");
		strcpy(sql_dailydata[row].centre4,	argv[10] ? argv[10] : "");
		strcpy(sql_dailydata[row].centre5,	argv[11] ? argv[11] : "");
		strcpy(sql_dailydata[row].centre6,	argv[12] ? argv[12] : "");
		strcpy(sql_dailydata[row].flag,		argv[13] ? argv[13] : "");
	}
...
static void show_form(void)
{
	...
	printf("每天数据\n");
	printf("</ul>\n");
	printf("<div class=\"title_right\"><span class=\"pull-right margin-bottom-5\"></span><strong>每天数据</strong></div>\n");
	...
	for (i = 0;i < row;i++)
	{
		...
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].datatime);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].polid);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].pol_min);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].pol_avg);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].pol_max);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].pol_col);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].centre1);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].centre2);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].centre3);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].centre4);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].centre5);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].centre6);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailydata[i].flag);
		printf("</tr>\n");
