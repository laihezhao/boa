...
static POLSALARM sql_polsalarm[1];
...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_polsalarm[0].id,		argv[0] ? argv[0] : "");
		strcpy(sql_polsalarm[0].polid,		argv[1] ? argv[1] : "");
		strcpy(sql_polsalarm[0].devname,	argv[2] ? argv[2] : "");
		strcpy(sql_polsalarm[0].devaddr,	argv[3] ? argv[3] : "");
		strcpy(sql_polsalarm[0].funcode,	argv[4] ? argv[4] : "");
		strcpy(sql_polsalarm[0].datatype,	argv[5] ? argv[5] : "");
		strcpy(sql_polsalarm[0].regaddr,	argv[6] ? argv[6] : "");
		strcpy(sql_polsalarm[0].reglen,		argv[7] ? argv[7] : "");
		strcpy(sql_polsalarm[0].databit,	argv[8] ? argv[8] : "");
		strcpy(sql_polsalarm[0].alarmtype,	argv[9] ? argv[9] : "");
	}
...
static int select_table(int n)
{
	...
	snprintf(sql,sizeof(sql),"SELECT id,polid,devname,devaddr,funcode,datatype,regaddr,reglen,\
			databit,alarmtype FROM %s where ID = %d ;",TABLENAME,n);
...
static void show_update(void)
{
	...
	printf("<td><input type=\"text\" name=\"update_alarmtype\" style=\"width:110px;\" value=\"%s\" class=\"inputxt\" \
			datatype=\"n1-2\" nullmsg=\"请输入报警类型！\" errormsg=\"报警类型！\" /></td>\n",sql_polsalarm[0].alarmtype);
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">报警类型！</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
