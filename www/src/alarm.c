...
static void init_memory(void)
{
	memset(sql_alarm, 0, sizeof(sql_alarm));
}
...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_alarm[row].id,	 argv[0] ? argv[0] : "");
		strcpy(sql_alarm[row].alarmtime, argv[1] ? argv[1] : "");
		strcpy(sql_alarm[row].alarmtype, argv[2] ? argv[2] : "");
		strcpy(sql_alarm[row].polid,	 argv[3] ? argv[3] : "");
		strcpy(sql_alarm[row].alarmvalue,argv[4] ? argv[4] : "");
		strcpy(sql_alarm[row].centre1,	 argv[5] ? argv[5] : "");
		strcpy(sql_alarm[row].centre2,   argv[6] ? argv[6] : "");
		strcpy(sql_alarm[row].centre3,   argv[7] ? argv[7] : "");
		strcpy(sql_alarm[row].centre4,   argv[8] ? argv[8] : "");
		strcpy(sql_alarm[row].centre5,   argv[9] ? argv[9] : "");
		strcpy(sql_alarm[row].centre6,   argv[10] ? argv[10] : "");
		strcpy(sql_alarm[row].flag,	 argv[11] ? argv[11] : "");
	}
...
static void show_form(void)
{
	...
	printf("报警信息\n");
	printf("</ul>\n");
	printf("<div class=\"title_right\"><span class=\"pull-right margin-bottom-5\"></span><strong>报警信息</strong></div>\n");

	printf("<table class=\"table table-bordered table-striped table-hover\">\n");
	printf("<tbody>\n");
	printf("<tr align=\"center\">\n");
	printf("<td nowrap=\"nowrap\"><strong>序号</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>报警时间</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>报警类型</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>仪表ID</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>报警值</strong></td>\n");
	;
	...
	for (i = 0;i < row;i++)
	{
		...
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_alarm[i].alarmtime);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_alarm[i].alarmtype);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_alarm[i].polid);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_alarm[i].alarmvalue);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_alarm[i].centre1);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_alarm[i].centre2);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_alarm[i].centre3);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_alarm[i].centre4);
