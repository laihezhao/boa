...
static char sql_POLVALUE[MAX_BUF_SIZE];
static char sql_UPVALUE[MAX_BUF_SIZE];
static char sql_LOWVALUE[MAX_BUF_SIZE];
static char sql_K[MAX_BUF_SIZE];
static char sql_B[MAX_BUF_SIZE];
static char sql_ALARMTYPE[MAX_BUF_SIZE];
static char sql_FLAG[MAX_BUF_SIZE];
...
static SITEPOLS sql_sitepols[100];
...
static void init_memory(void)
{
	memset(sql_sitepols, 0, sizeof(sql_sitepols));
	...
	memset(sql_POLVALUE, 0, sizeof(sql_POLVALUE));
	memset(sql_UPVALUE,  0, sizeof(sql_UPVALUE));
	memset(sql_LOWVALUE, 0, sizeof(sql_LOWVALUE));
	memset(sql_K,	     0, sizeof(sql_K));
	memset(sql_B,	     0, sizeof(sql_B));
	memset(sql_ALARMTYPE,0, sizeof(sql_ALARMTYPE));
	memset(sql_FLAG,     0, sizeof(sql_FLAG));
...
static void update_value(void)
{
	...
	get_form_value("update_polvalue", sql_POLVALUE, sizeof(sql_POLVALUE));
	...
	get_form_value("update_k", 	  sql_K, 	sizeof(sql_K));
	get_form_value("update_b",	  sql_B,	sizeof(sql_B));
	get_form_value("update_alarmtype",sql_ALARMTYPE,sizeof(sql_ALARMTYPE));
	get_form_value("update_flag",     sql_FLAG,     sizeof(sql_FLAG));
}
...
static void insert_value(void)
{
	...
	get_form_value("POLVALUE", sql_POLVALUE, sizeof(sql_POLVALUE));
	get_form_value("UPVALUE",  sql_UPVALUE,  sizeof(sql_UPVALUE));
	get_form_value("LOWVALUE", sql_LOWVALUE, sizeof(sql_LOWVALUE));
	get_form_value("K", 	   sql_K, 	 sizeof(sql_K));
	get_form_value("B",	   sql_B,	 sizeof(sql_B));
	get_form_value("ALARMTYPE",sql_ALARMTYPE,sizeof(sql_ALARMTYPE));
	get_form_value("FLAG",     sql_FLAG,     sizeof(sql_FLAG));
...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_sitepols[row].id,		argv[0] ? argv[0] : "");
		strcpy(sql_sitepols[row].polid,		argv[1] ? argv[1] : "");
		strcpy(sql_sitepols[row].polname,	argv[2] ? argv[2] : "");
		strcpy(sql_sitepols[row].devname,	argv[3] ? argv[3] : "");
		strcpy(sql_sitepols[row].devaddr,	argv[4] ? argv[4] : "");
		strcpy(sql_sitepols[row].funcode,	argv[5] ? argv[5] : "");
		strcpy(sql_sitepols[row].datatype,	argv[6] ? argv[6] : "");
		strcpy(sql_sitepols[row].regaddr,	argv[7] ? argv[7] : "");
		strcpy(sql_sitepols[row].reglen,	argv[8] ? argv[8] : "");
		strcpy(sql_sitepols[row].databit,	argv[9] ? argv[9] : "");
		strcpy(sql_sitepols[row].polvalue,	argv[10] ? argv[10] : "");
		strcpy(sql_sitepols[row].upvalue,	argv[11] ? argv[11] : "");
		strcpy(sql_sitepols[row].lowvalue,	argv[12] ? argv[12] : "");
		strcpy(sql_sitepols[row].k,	        argv[13] ? argv[13] : "");
		strcpy(sql_sitepols[row].b,	        argv[14] ? argv[14] : "");
		strcpy(sql_sitepols[row].alarmtype,	argv[15] ? argv[15] : "");
		strcpy(sql_sitepols[row].flag,	        argv[16] ? argv[16] : "");
	}
...
static int update_table(void)
{
	...
	snprintf(sql,sizeof(sql),"UPDATE %s SET polid=\'%s\',polname=\'%s\',devname=\'%s\',devaddr=%d,\
			funcode=\'%s\',datatype=%d,regaddr=%d, reglen=%d,databit=%d,upvalue=\'%s\',\
			lowvalue=\'%s\',K=\'%s\',B=\'%s\',alarmtype=%d where ID=%d;",TABLENAME,sql_POLID,sql_POLNAME,\
			sql_DEVNAME,atoi(sql_DEVADDR),sql_FUNCODE,atoi(sql_DATATYPE),atoi(sql_REGADDR),atoi(sql_REGLEN),\
			atoi(sql_DATABIT),sql_UPVALUE,sql_LOWVALUE,sql_K, sql_B,atoi(sql_ALARMTYPE),atoi(sql_ID));
...
static int insert_table(void)
{
	...
	snprintf(sql,sizeof(sql),"INSERT INTO %s (id,polid,polname,devname,devaddr,funcode,datatype,regaddr,reglen,\
							databit,upvalue,lowvalue,K,B,alarmtype) \
						VALUES (%d,\'%s\',\'%s\',\'%s\',%d,\'%s\',%d,%d,%d,%d,\'%s\',\'%s\',\'%s\',\'%s\',%d);",\
					TABLENAME,atoi(sql_ID),sql_POLID,sql_POLNAME,sql_DEVNAME,atoi(sql_DEVADDR), sql_FUNCODE,\
					atoi(sql_DATATYPE),atoi(sql_REGADDR),atoi(sql_REGLEN), atoi(sql_DATABIT),sql_UPVALUE,\
					sql_LOWVALUE,sql_K,sql_B,atoi(sql_ALARMTYPE));
...
static void show_form(void)
{
	...
	printf("window.location = \"/cgi-bin/update_sitepols.cgi?id=\" + id;\n");
        printf("}\n");
    	printf("</script>\n");
	...
	printf("});\n");
        ...
        printf("window.location = \"/cgi-bin/sitepols.cgi?Id=\" + Id;\n");
        printf("}\n");
        printf("}\n");
	printf("</script>\n");
	...
	printf("<div class=\"title_right\"><span class=\"pull-right margin-bottom-5\"> \
			...
			<i class=\"icon-plus icon-white\"></i> 添加因子参数</a> \
			</span><strong>因子参数</strong></div>\n");
	...
	printf("<form class=\"registerform\" action=\"/cgi-bin/sitepols.cgi\">\n");
	...
	printf("<td><input type=\"text\" name=\"POLID\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
			datatype=\"*1-20\" nullmsg=\"请输入仪表ID！\" errormsg=\"仪表ID1-20任意字符!\" /></td>\n");
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">仪表ID1-20任意字符!</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
	printf("</tr>\n");
	printf("</table>\n");
	...
	printf("<table class=\"table table-bordered\" style=\"margin-bottom:0px;\">\n");
	printf("<tr>\n");
	printf("<td width=\"15%%\" align=\"right\" nowrap=\"nowrap\" bgcolor=\"#f1f1f1\">仪表名称：</td>\n");
	printf("<td><input type=\"text\" name=\"POLNAME\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
			datatype=\"*1-30\" nullmsg=\"请输入仪表名称！\" errormsg=\"仪表名称1-30位任意字符！\" /></td>\n");
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">仪表名称1-30位任意字符！</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
	...
	printf("<table class=\"table table-bordered\" style=\"margin-bottom:0px;\">\n");
	printf("<tr>\n");
	printf("<td width=\"15%%\" align=\"right\" nowrap=\"nowrap\" bgcolor=\"#f1f1f1\">量程上限：</td>\n");
	printf("<td><input type=\"text\" name=\"UPVALUE\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
        ...
        printf("<div class=\"info\"><span class=\"Validform_checktip\">量程上限！</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
	printf("<td width=\"15%%\" align=\"right\" nowrap=\"nowrap\" bgcolor=\"#f1f1f1\">量程下限：</td>\n");
	printf("<td><input type=\"text\" name=\"LOWVALUE\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
			datatype=\"*1-32\" nullmsg=\"请输入量程下限！\" errormsg=\"量程下限!\" /></td>\n");
        printf("<td>\n");
	...
	printf("<table class=\"table table-bordered\" style=\"margin-bottom:0px;\">\n");
	printf("<tr>\n");
	printf("<td width=\"15%%\" align=\"right\" nowrap=\"nowrap\" bgcolor=\"#f1f1f1\">K：</td>\n");
	printf("<td><input type=\"text\" name=\"K\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
			datatype=\"*1-32\" nullmsg=\"请输入斜率！\" errormsg=\"斜率！\" /></td>\n");
        printf("<td>\n");
        printf("<div class=\"info\"><span class=\"Validform_checktip\">斜率！</span> \
			<span class=\"dec\"><s class=\"dec1\">&#9670;</s><s class=\"dec2\">&#9670;</s></span></div>\n");
        printf("</td>\n");
	printf("<td width=\"15%%\" align=\"right\" nowrap=\"nowrap\" bgcolor=\"#f1f1f1\">B：</td>\n");
	printf("<td><input type=\"text\" name=\"B\" style=\"width:110px;\" value=\"\" class=\"inputxt\" \
	...
	printf("<td><strong>量程上限</strong></td>\n");
	printf("<td><strong>量程下限</strong></td>\n");
	printf("<td><strong>K(斜率)</strong></td>\n");
	printf("<td><strong>B(截距)</strong></td>\n");
	...
	for (i = 0;i < row;i++)
	{
		...
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].polid);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].polname);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].devname);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].devaddr);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].funcode);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].datatype);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].regaddr);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].reglen);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].databit);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].upvalue);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].lowvalue);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].k);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].b);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_sitepols[i].alarmtype);
	        printf("<td nowrap=\"nowrap\"><a href=\"javascript:void(0)\" onclick=\"Dele(%s)\">删除</a> \
						<a href=\"javascript:void(0)\" onclick=\"Edit(%s)\">修改</a></td>\n",\
						sql_sitepols[i].id,sql_sitepols[i].id);
		printf("</tr>\n");
	}
