...
static void init_memory(void)
{
	memset(sql_fault, 0, sizeof(sql_fault));
}
...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_fault[row].id,	 argv[0] ? argv[0] : "");
		strcpy(sql_fault[row].faulttime, argv[1] ? argv[1] : "");
		strcpy(sql_fault[row].faulttype, argv[2] ? argv[2] : "");
		strcpy(sql_fault[row].sbid,	 argv[3] ? argv[3] : "");
		strcpy(sql_fault[row].faultvalue,argv[4] ? argv[4] : "");
		strcpy(sql_fault[row].centre1,   argv[5] ? argv[5] : "");
		strcpy(sql_fault[row].centre2,   argv[6] ? argv[6] : "");
		strcpy(sql_fault[row].centre3,   argv[7] ? argv[7] : "");
		strcpy(sql_fault[row].centre4,   argv[8] ? argv[8] : "");
		strcpy(sql_fault[row].centre5,   argv[9] ? argv[9] : "");
		strcpy(sql_fault[row].centre6,   argv[10] ? argv[10] : "");
		strcpy(sql_fault[row].flag,	 argv[11] ? argv[11] : "");
	}
...
static void show_form(void)
{
	...
	printf("故障信息\n");
	printf("</ul>\n");
	printf("<div class=\"title_right\"><span class=\"pull-right margin-bottom-5\"></span><strong>故障信息</strong></div>\n");

	printf("<table class=\"table table-bordered table-striped table-hover\">\n");
	printf("<tbody>\n");
	printf("<tr align=\"center\">\n");
	printf("<td nowrap=\"nowrap\"><strong>序号</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>故障时间</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>故障类型</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>设备ID</strong></td>\n");
	printf("<td nowrap=\"nowrap\"><strong>故障值</strong></td>\n");
	...
	for (i = 0;i < row;i++)
	{
		...
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_fault[i].faulttime);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_fault[i].faulttype);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_fault[i].sbid);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_fault[i].faultvalue);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_fault[i].centre1);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_fault[i].centre2);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_fault[i].centre3);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_fault[i].centre4);
