...
#define TABLENAME "dailyruntime"
#define COLUMN_SIZE 20

static sqlite3 *db;
static int row = 0;

typedef struct Dailyruntime
{
	char id[32];
	char sbid[32];
	char datatime[32];
...
static DAILYRUNTIME sql_dailyruntime[100];
...
static void init_memory(void)
{
	memset(sql_dailyruntime,0, sizeof(sql_dailyruntime));
}
...
static int select_callback(void *data,int argc,char **argv,char **azColName)
{
	...
	{
		strcpy(sql_dailyruntime[row].id,	argv[0] ? argv[0] : "");
		strcpy(sql_dailyruntime[row].sbid,	argv[1] ? argv[1] : "");
		strcpy(sql_dailyruntime[row].datatime,	argv[2] ? argv[2] : "");
		strcpy(sql_dailyruntime[row].runtime,	argv[3] ? argv[3] : "");
		strcpy(sql_dailyruntime[row].faulttime,	argv[4] ? argv[4] : "");
		strcpy(sql_dailyruntime[row].sp_avg,	argv[5] ? argv[5] : "");
		strcpy(sql_dailyruntime[row].centre1,	argv[6] ? argv[6] : "");
		strcpy(sql_dailyruntime[row].centre2,	argv[7] ? argv[7] : "");
		strcpy(sql_dailyruntime[row].centre3,	argv[8] ? argv[8] : "");
		strcpy(sql_dailyruntime[row].centre4,	argv[9] ? argv[9] : "");
		strcpy(sql_dailyruntime[row].centre5,	argv[10] ? argv[10] : "");
		strcpy(sql_dailyruntime[row].centre6,	argv[11] ? argv[11] : "");
		strcpy(sql_dailyruntime[row].flag,	argv[12] ? argv[12] : "");
	}
...
static void show_form(void)
{
	...
	printf("运行时间\n");
	printf("</ul>\n");
	printf("<div class=\"title_right\"><span class=\"pull-right margin-bottom-5\"></span><strong>运行时间</strong></div>\n");
	...
	printf("<td nowrap=\"nowrap\"><strong>运行时间</strong></td>\n");
	printf("<td><strong>故障时间</strong></td>\n");
	printf("<td><strong>设定平均值</strong></td>\n");
	...
	{
		printf("<tr align=\"center\">\n");
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].id);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].sbid);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].datatime);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].runtime);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].faulttime);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].sp_avg);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].centre1);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].centre2);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].centre3);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].centre4);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].centre5);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].centre6);
		printf("<td nowrap=\"nowrap\"> %s </td>\n",sql_dailyruntime[i].flag);
		printf("</tr>\n");
